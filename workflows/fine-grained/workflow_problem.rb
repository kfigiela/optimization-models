...
def schema 
  {
      ...
      transfer_latency: DataGenerator::Vector.new([:storages, :providers]),
      ...
      exec_time: DataGenerator::Vector.new([:tasks, :instances]),
      layers: { 
...
def run_optimization!(params)
  ...
  workflow = YAML.load_file('montage.8.0.yaml')
